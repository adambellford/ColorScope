/*
  DIY фазово-контрастная подстветка для микроскопа
  
  Скетч позволяет управлять цветом светодиодной ленты с трех потенциометров.
  Код работает на Arduino UNO с лентой WS2811, но немного изменив,
  он подойдет и к другим комбинациям железа.
